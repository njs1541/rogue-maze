...
class NeonRankSystem {
    ...
    async addRankRecord(name, score, room, kills, weapon) {
        ...
        if (this.isOnline && this.db) {
            try {
                // [개선] 파이어베이스 보안 규칙 미설정 등으로 인한 무한 펜딩 방지용 5초 타임아웃 프로미스 레이스 적용
                const firestorePromise = this.db.collection('rankings').add(record);
    ...
    async getTopRankings(limitCount = 10) {
        ...
        if (this.isOnline && this.db) {
            try {
                // [개선] 무한 대기 방지 5초 타임아웃 쿼리 레이스 적용
                const firestorePromise = this.db.collection('rankings')
                    .orderBy('score', 'desc')
                    .orderBy('room', 'desc')
                    .limit(limitCount)
                    .get();
                ...
                // 온라인 랭킹과 로컬 랭킹 병합 정렬 (로컬에만 임시 저장된 고득점이 있다면 합산 노출)
                const merged = this.mergeWithLocal(rankings, limitCount);
                return merged;
            } catch (error) {
    ...
    saveToLocal(record) {
        // Firestore 필드 호환을 위해 서버 타임스탬프 대체
        ...
        try {
            ...
            localStorage.setItem('neon_rogue_rankings', JSON.stringify(localData.slice(0, 200)));
        } catch (e) {
