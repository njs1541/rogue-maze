        ...
        if (this.isSlashActive) {
            ...
            for (let angle of drawAngles) {
                if (isWhip) {
                    ...
                    let scaleFactor = Math.sqrt(this.atk / 10);
                    let isCrudeSword = (this.equippedWeapons[0] === 'crude_sword');

                    if (!isCrudeSword && isAdvanced) {
                        // 1) 플라즈마 세이버 (Advanced) -> 무조건 베기(Slash) 보라/시안 아크 궤적
                        let sRadius = 130;
                        let start = angle - 0.9;
                        let end = angle + 0.9;

                        let grad = ctx.createRadialGradient(0, 0, sRadius - 15, 0, 0, sRadius + 15);
                        ...
                        // 2) 조잡한 진동검 (Crude) -> 무조건 찌르기(Thrust) 피스톤 노란색 진동선 (단일 사출 강제)
                        if (!this._thrustRenderedThisFrame) {
                            ...
                            if (startDist < endDist) {
                                let jitterX = (Math.random() - 0.5) * 4.5;
                                let jitterY = (Math.random() - 0.5) * 4.5;
                                let singleAngle = this.slashAngle || this.angle;

                                ctx.save();
                                ctx.beginPath();
                                ctx.moveTo(Math.cos(singleAngle) * startDist, Math.sin(singleAngle) * startDist);
                                ctx.lineTo(Math.cos(singleAngle) * endDist + jitterX, Math.sin(singleAngle) * endDist + jitterY);
                                ...
                                // 미세 주황 오버레이 궤적 진동선
                                ctx.beginPath();
                                ctx.moveTo(Math.cos(singleAngle + 0.03) * startDist, Math.sin(singleAngle + 0.03) * startDist);
                                ctx.lineTo(Math.cos(singleAngle + 0.03) * endDist, Math.sin(singleAngle + 0.03) * endDist);
                                ctx.strokeStyle = 'rgba(235, 120, 20, 0.45)';
                                ctx.lineWidth = 1.0;
                                ctx.stroke();
                                ctx.restore();
                            }
