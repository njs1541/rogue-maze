...
const filePath = path.join(__dirname, '..', 'discussion_notes.md');
console.log('Target file path:', filePath);

let content = fs.readFileSync(filePath, 'utf8');

// 1. 1차 제작 테이블 교체
const oldTable = `### 🧪 1차 제작: 조잡한 무기 (Crude Weapons)
...
const newTable = `### 🧪 1차 제작: 조잡한 무기 (Crude Weapons)
부품 조합을 통해 1차적으로 제작하는 기본 무기 리스트 및 매핑입니다. (내부 ID 변경 및 레시피 확정)
...
// 공백 문자의 차이로 매칭이 안 되는 것을 방지하기 위해 정규화 수행 후 치환 시도
const normalize = str => str.replace(/\r\n/g, '\n').trim();

let normContent = content.replace(/\r\n/g, '\n');
let normOldTable = normalize(oldTable);
let normNewTable = newTable.replace(/\r\n/g, '\n');

if (normContent.includes(normOldTable)) {
    normContent = normContent.replace(normOldTable, normNewTable);
    console.log('1차 제작 테이블 교체 완료');
} else {
    console.warn('경고: 1차 제작 테이블 매칭 실패. 정밀 매칭을 시도합니다.');
    // 단순 줄단위 교체도 안 될 경우 테이블 시작 부분을 기반으로 찾음
    const tableIndex = normContent.indexOf('### 🧪 1차 제작: 조잡한 무기');
    if (tableIndex !== -1) {
        const nextHeaderIndex = normContent.indexOf('### 🚀 2차 진화: 하이테크 SF 무기', tableIndex);
        if (nextHeaderIndex !== -1) {
            const tablePart = normContent.substring(tableIndex, nextHeaderIndex);
            normContent = normContent.replace(tablePart, normNewTable + '\n\n');
            console.log('1차 제작 테이블 범위 기반 교체 완료');
        }
    }
}

// 2. 비주얼 상세 기획 부분 교체
const visualHeader = '## 🎨 [2단계] 무기별 비주얼 및 Canvas 렌더링 상세 기획';
const visualIndex = normContent.indexOf(visualHeader);

const newVisualSection = `## 🎨 [2단계] 무기별 비주얼 및 Canvas 렌더링 상세 기획
...
if (visualIndex !== -1) {
    const prevContent = normContent.substring(0, visualIndex);
    normContent = prevContent + newVisualSection;
    console.log('비주얼 상세 기획 영역 교체 완료');
} else {
    // 만약 기존에 헤더가 없었다면 그냥 끝에 덧붙임
    normContent += '\n\n' + newVisualSection;
    console.log('비주얼 상세 기획 영역 추가 완료 (기존 헤더 없음)');
}

// 다시 CRLF 형식으로 복원하여 쓰기
const finalContent = normContent.replace(/\n/g, '\r\n');
fs.writeFileSync(filePath, finalContent, 'utf8');
console.log('discussion_notes.md 저장 성공');
